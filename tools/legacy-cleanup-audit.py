#!/usr/bin/env python3
"""Read-only repository audit for Hangar18 legacy/runtime cleanup."""
...
DIAGNOSTIC_RUNTIME_PATHS = {'src/Admin/LegacyCleanupAuditAdminController.php'}
...
def audit(root: pathlib.Path) -> dict:
    ...
    for path in iter_files(root):
        ...
        if not count:
            continue
        entry = {'path': rel, 'matches': count, 'shim': path.name in SHIM_NAMES}
        if rel in DIAGNOSTIC_RUNTIME_PATHS:
            diagnostic_mentions.append(entry)
        else:
            whatif_files.append(entry)
    ...
    return {
        ...
        'whatif_diagnostic_mentions': sorted(diagnostic_mentions, key=lambda item: item['path']),
...
def main() -> int:
    ...
    parser.add_argument('--require-no-active-whatif', action='store_true')
    ...
    if args.require_no_active_whatif and result['whatif_runtime_file_count']:
        errors.append(f"Active WhatIf runtime files found: {result['whatif_files']}")
    for error in errors:
