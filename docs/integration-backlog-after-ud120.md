...
| I9-EVIDENCE-1 — Manifest validator | ✅ QA PASS | Struktur, gates, evidence, deterministisk overall status og release-gate-mode. |
| I9-EVIDENCE-2 — Manifest initializer | ✅ QA PASS | Ny build seedes sikkert med alle otte gates `PENDING`; kan ikke fremstille acceptance. |
| I9-EVIDENCE-3 — JSON/Markdown rapport | ✅ QA PASS | Maskinlæsbar validator-output og kompakt human-readable evidence-status. |
| I9-EVIDENCE-4 — Actions release-gate | ✅ QA PASS | `workflow_dispatch`-only, `contents: read`, validator-resultat som artifact. |
| I9-EVIDENCE-5 — Build/environment binding | ✅ QA PASS | Manifest kan bindes til forventet commit-SHA, pluginversion og normaliseret staging-target. |
| I9-EVIDENCE-6 — Source version resolution | ✅ QA PASS | Blank workflow-version læses fra `Hangar18_Manager::VERSION`; aktuel baseline 0.8.39. |
| I9-EVIDENCE-7 — Read-only gate recorder | ✅ QA PASS | Én gate transformeres ad gangen; PASS kræver evidence; kildefilen ændres aldrig; overall status genberegnes. |
| I9-EVIDENCE-8 — Evidence integrity index | ✅ QA PASS | Lokale evidence-filer bindes med SHA-256/bytes; missing/traversal fejler; eksterne refs mærkes ikke-lokalt-verificeret. |
| I9-EVIDENCE-9 — Integrity i Actions gate | ✅ QA PASS | Dispatch-run producerer validation + integrity som artifact; valgfri `require_all_local`. |
...
- `tests/Architecture/i9-evidence-gate-templates-contract.sh`.
