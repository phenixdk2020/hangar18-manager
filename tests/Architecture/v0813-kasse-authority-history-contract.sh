...
require_contains "$CTRL" "callback.name === 'editorHistoryRecordNow'" 'derived history capture is not deferred during restore'
...
if grep -F 'createInitialBox($row);' "$LAYOUT" >/dev/null; then
...
require_contains "$NEST" "text: 'v0.8.14'" 'visible Kasse runtime badge was not advanced to v0.8.14'
...
echo 'v0.8.13-v0.8.14 Kasse authority/history contract: PASS'
