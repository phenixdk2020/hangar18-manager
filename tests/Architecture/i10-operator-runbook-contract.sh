...
LEDGER='docs/i10-stage-ledger-template.md'

for file in "$PLAN" "$GATE" "$PREFLIGHT" "$ACCEPTANCE" "$CHECKLIST" "$TARGETS" "$RUNBOOK" "$BLOCKERS" "$TEMPLATE" "$LEDGER"; do
...
require_contains "$LEDGER" '| 2 | core | `hjem` |' 'stage ledger Hjem order missing'
require_contains "$LEDGER" '| 3 | core | `om-foreningen` |' 'stage ledger Om order missing'
require_contains "$LEDGER" '| 4 | core | `kontakt` |' 'stage ledger Kontakt order missing'
require_contains "$LEDGER" '| 5 | core | `bliv-medlem` |' 'stage ledger Bliv medlem order missing'
...
for key in desktop-compare tablet-compare mobile-compare save-flow preview-flow revision-flow rollback-flow; do
  ...
  require_contains "$LEDGER" "$key" "stage ledger acceptance check missing: $key"
done
...
require_contains "$LEDGER" 'Shadow SourceHash' 'stage ledger shadow hash missing'
require_contains "$LEDGER" 'Acceptance SourceHash' 'stage ledger acceptance hash missing'

# Documentation must not imply that I10 is currently executable.
if grep -Ei 'cutover (nu|now)|public mutation (er|is) available|Executable=true|PublicMutationAvailable=true' "$RUNBOOK" "$BLOCKERS" "$TEMPLATE" "$LEDGER" >/dev/null; then
