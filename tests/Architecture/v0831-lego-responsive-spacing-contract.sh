...
require_contains "$JS" "device + '.Margin.X'" 'shared device X/Y margin control path generation is missing'
require_contains "$JS" "device + '.Gap.X'" 'shared device X/Y gap control path generation is missing'
