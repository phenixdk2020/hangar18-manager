...
HISTORY_SPEC='tests/Architecture/browser/lego-consolidation-history-v0835.spec.cjs'
ADMIN='assets/admin.js'

for file in "$NESTING" "$SPACING" "$RESPONSIVE" "$INTERACTION" "$SPEC" "$HISTORY_SPEC" "$ADMIN"; do
...
# Sequential Undo/Redo must prove one logical action per spacing/design/state checkpoint.
require_contains "$HISTORY_SPEC" 'spacing design and state Undo Redo one logical action at a time on nested Kasse' 'sequential combined Undo/Redo test is missing'
require_contains "$HISTORY_SPEC" "expect(await page.evaluate(() => window.__v0835History.state())).toEqual({ index:3, entries:4 });" 'one-action-per-checkpoint assertion is missing'
require_contains "$HISTORY_SPEC" "{ parent:'auto-1', gap:27, radius:18, active:'None' }" 'first Undo isolation assertion is missing'
require_contains "$HISTORY_SPEC" "{ parent:'auto-1', gap:27, radius:7, active:'None' }" 'second Undo isolation assertion is missing'
require_contains "$HISTORY_SPEC" "{ parent:'auto-1', gap:12, radius:7, active:'None' }" 'third Undo isolation assertion is missing'
require_contains "$HISTORY_SPEC" "{ parent:'auto-1', gap:27, radius:18, active:'ScaleDown' }" 'Redo final-state assertion is missing'

# The v0.8.35 test-only slice must not introduce a WordPress persistence store or a second production history stack.
if grep -Ei 'update_option|add_option|delete_option|admin_post_|wp_ajax_|localStorage|sessionStorage|undoStack|redoStack|historyEntries|editorHistoryEntries' "$SPEC" "$HISTORY_SPEC" >/dev/null; then
  echo 'FAIL: consolidation tests introduced or assume a parallel WordPress persistence/history stack'
  exit 1
fi
