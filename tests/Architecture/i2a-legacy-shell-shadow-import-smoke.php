...
// Simulate a repository bug/misbehavior that changes global assignment during save.
$assignmentDriftRepo = new class implements SiteTemplateRepository {
    public array $templates = [];
    public array $assignments = ['header'=>'public-header','footer'=>'public-footer'];
    public function all(): array { return $this->templates; }
    public function get(string $templateId): ?array { return $this->templates[$templateId] ?? null; }
    public function save(array $template): array {
        $this->templates[(string) $template['Id']] = $template;
        if (($template['Kind'] ?? '') === 'header') { $this->assignments['header'] = 'MUTATED'; }
        return $template;
    }
    public function delete(string $templateId): void { unset($this->templates[$templateId]); }
    public function assignGlobal(string $kind, ?string $templateId): void {}
...
$driftService = new LegacyShellShadowImportService($assignmentDriftRepo, new SiteTemplateValidator(new PageSchemaValidator()));
$driftRejected = false;
try {
    $driftService->import($snapshot);
} catch (RuntimeException $exception) {
    $driftRejected = true;
}
i2aAssert($driftRejected, 'Any global assignment drift during shadow import must fail closed.');
i2aAssert(count($assignmentDriftRepo->templates) === 0, 'Assignment drift must roll back newly created shadow templates.');
