...
grep -F 'function finishNest(beforeKeys, type, boxKey)' "$JS" >/dev/null
...
# Existing rows are dragged by jQuery UI Sortable, so the enhancer must listen
# to those events rather than relying only on native HTML5 drag/drop.
grep -F "sortstart.h18UdBoxNesting" "$JS" >/dev/null
grep -F "sort.h18UdBoxNesting" "$JS" >/dev/null
grep -F "sortstop.h18UdBoxNesting" "$JS" >/dev/null
...
# Kasse must expose an explicit inside drop target instead of only before/after
...
node --check "$JS"
echo 'v0.8.8 existing element -> Kasse drop contract: PASS'
