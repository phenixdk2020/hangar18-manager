...
for file in "$MODEL" "$CTRL" "$SPACING_CTRL" "$JS" "$CSS"; do
...
if grep -Ei 'update_option|add_option|delete_option|admin_post_|wp_ajax_|localStorage|sessionStorage' "$CTRL" "$JS" >/dev/null; then
...
if grep -Ei 'wp_update_post|wp_insert_post|update_post_meta|delete_post_meta|template_redirect|wp_head|wp_footer|wp_nav_menu' "$MODEL" "$CTRL" "$JS" >/dev/null; then
