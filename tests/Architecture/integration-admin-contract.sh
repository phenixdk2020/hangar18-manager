...
grep -F 'Elementbibliotek' src/Admin/IntegrationAdminBootstrap.php >/dev/null
...
grep -F "backlogRow('UX-2','Færdig'" src/Admin/IntegrationAdminBootstrap.php >/dev/null
...
if grep -RInE 'ultimate-designer-(admin|menu-admin|menu-pages|side-health|asset-admin|portability|permissions|ai|qa|conversion|layout-tools|element-library)\.(js|css)' src --include='*.php' | grep -vE '^src/Admin/' >/dev/null; then
