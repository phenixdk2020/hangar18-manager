...
for controller in \
  ...
  src/Admin/PortabilityAdminController.php; do
...
grep -F "backlogRow('I5','Færdig'" src/Admin/IntegrationAdminBootstrap.php >/dev/null
grep -F "backlogRow('I6','Aktiv'" src/Admin/IntegrationAdminBootstrap.php >/dev/null
...
grep -F 'PortabilityAdminController::register();' src/Admin/IntegrationAdminBootstrap.php >/dev/null
grep -F 'v0.7.4 – Ultimate Designer integration dashboard' assets/admin.css >/dev/null

if grep -RInE 'ultimate-designer-(admin|menu-admin|menu-pages|side-health|asset-admin|portability)\.(js|css)' src --include='*.php' | grep -vE '^src/Admin/' >/dev/null; then
  echo 'FAIL: Ultimate Designer admin asset referenced outside Admin namespace'; exit 1
fi

echo 'Ultimate Designer integration admin safety contract I1-I6: PASS'
