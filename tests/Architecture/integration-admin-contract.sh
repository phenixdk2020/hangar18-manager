...
if grep -E "add_action\('(wp|init|template_redirect|wp_head|wp_footer)" src/Admin/IntegrationAdminBootstrap.php >/dev/null; then echo 'FAIL: integration dashboard registers a frontend/runtime hook'; exit 1; fi
for controller in src/Admin/SiteTemplateAdminController.php src/Admin/MenuAdminController.php src/Admin/SideHealthAdminController.php; do
  if grep -E "add_action\('(wp|init|template_redirect|wp_head|wp_footer)" "$controller" >/dev/null; then echo "FAIL: admin controller registers frontend/runtime hook: $controller"; exit 1; fi
done
