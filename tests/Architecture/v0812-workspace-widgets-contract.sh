...
grep -F "const STORAGE_KEY = 'hangar18UltimateDesignerWorkspaceWidgetsV0812';" "$JS" >/dev/null
...
# Mobile/tablet keeps the existing stacked editor rather than forcing collapsed rails.
...
node --check "$JS"
echo 'v0.8.12 collapsible workspace widgets contract: PASS'
