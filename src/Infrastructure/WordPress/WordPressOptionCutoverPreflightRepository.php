...
final class WordPressOptionCutoverPreflightRepository
{
    ...
    public function save(string $slug, array $preflight, array $signed, int $userId): array
    {
        ...
        if ($slug === '' || ($preflight['Slug'] ?? '') !== $slug) {
            throw new RuntimeException('Cutover preflight slug mismatch.');
        }
        if (empty($preflight['EligibleForFutureCutover']) || !empty($preflight['Blockers'])) {
            throw new RuntimeException('Blocked cutover preflight cannot be persisted as ready.');
        }
        if (!empty($preflight['Executable']) || !empty($preflight['PublicMutationAvailable'])) {
            throw new RuntimeException('Preflight repository only accepts non-executable snapshots.');
        }
        $token = trim((string) ($signed['token'] ?? ''));
        $hash = trim((string) ($signed['preflightHash'] ?? ''));
        $expires = (int) ($signed['expires'] ?? 0);
        if ($token === '' || $hash === '' || $expires <= time()) {
            throw new RuntimeException('Signed cutover preflight metadata is invalid or expired.');
        }
        $record = [
            'SchemaVersion' => '1.0',
            'Slug' => $slug,
        ...
        if ($ok === false && get_option(self::OPTION, []) !== $all) {
            throw new RuntimeException('Cutover preflight could not be persisted.');
        }
        return $record;
    }
}
