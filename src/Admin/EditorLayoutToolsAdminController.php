...
final class EditorLayoutToolsAdminController
{
    ...
    {
        $css = <<<'CSS'
#h18-page-sections-sortable[data-h18-v0810-kasse-runtime="1"] .h18-page-section-row.h18-v0810-child-source{display:none!important}
...
var BOX='Kasse',AUTO='Auto-kasser',timer=null,guard=false,boxDrag=null,containerClick=null;
...
function setLabel($r,v){var $f=ctl($r,'.h18-section-navigator-label').first();if($f.length){$f.val(v).trigger('input').trigger('change');}}
function setField($r,n,v){var $f=ctl($r,'[name$="['+n+']"]').first();if(!$f.length){return;}if($f.is(':checkbox')){$f.prop('checked',!!v).trigger('change');}else{$f.val(String(v)).trigger('input').trigger('change');}}
...
function refresh(){if(guard){return;}guard=true;$s.attr('data-h18-v0810-kasse-runtime','1');rows().removeClass('h18-v0810-child-source');rows().each(function(){var $r=$(this);if(isBox($r)){ensureBoxUi($r);}});rows().each(function(){var $r=$(this);if(isAuto($r)){ensureAutoUi($r);}});rows().each(function(){var $r=$(this),$p=byKey(parent($r));if(isBox($p)||isAuto($p)){$r.addClass('h18-v0810-child-source');}});guard=false;}
function schedule(ms){window.clearTimeout(timer);timer=window.setTimeout(refresh,typeof ms==='number'?ms:80);}
function finishContainer(before,target,side){window.setTimeout(function(){var $n=findNew(before,'container');if(!$n.length){schedule(50);return;}if(!isBox($n)){configBox($n);}if(!target){schedule(30);return;}var $t=byKey(target);if(!isBox($t)){schedule(30);return;}var $tp=byKey(parent($t));if(isAuto($tp)){setParent($n,key($tp));if(side==='left'){$n.insertBefore($t);}else{$n.insertAfter($t);}syncOrder();schedule(40);return;}var beforeGrid=snapshot(),$gridBtn=$('.h18-builder-palette-item[data-section-type="grid"]').not('[data-h18-layout-tool]').first();if(!$gridBtn.length){schedule(40);return;}$gridBtn.trigger('click');window.setTimeout(function(){var $g=findNew(beforeGrid,'grid');if(!$g.length){schedule(40);return;}configAuto($g);var gk=key($g);setParent($t,gk);setParent($n,gk);$g.insertBefore($t);if(side==='left'){$n.insertAfter($g);$t.insertAfter($n);}else{$t.insertAfter($g);$n.insertAfter($t);}syncOrder();schedule(60);},100);},120);}
$(document).on('click','.h18-v0810-edit',function(e){e.preventDefault();e.stopPropagation();var $r=byKey($(this).attr('data-h18-v0810-edit'));if(!$r.length){return;}$r.removeClass('h18-v0810-child-source');var $h=$r.children('.h18-page-section-header').first();($h.length?$h:$r).trigger('click');schedule(150);});
...
var mo=new MutationObserver(function(){if(!guard){schedule(100);}});mo.observe($s.get(0),{childList:true,subtree:true});
schedule(220);
});
JS;
