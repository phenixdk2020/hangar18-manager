...
/** Builds a non-executable future-cutover preflight. It never performs or exposes cutover. */
final class ConversionCutoverPreflightService
{
    ...
    public function build(
        ...
        if ($pageId <= 0) { $blockers[] = 'wordpress-page-id-missing'; }
        if ($permalink === '') { $blockers[] = 'wordpress-permalink-missing'; }
        ...
        if (!is_array($shadow)) {
            ...
            foreach ((array) ($drift['Blockers'] ?? []) as $blocker) { $blockers[] = (string) $blocker; }

            $acceptanceValid = $this->acceptance->isAccepted($acceptanceRecord, $sourceHash);
            if (!$acceptanceValid) {
                foreach ($this->acceptance->blockers($acceptanceRecord, $sourceHash) as $blocker) { $blockers[] = $blocker; }
            }
        }
