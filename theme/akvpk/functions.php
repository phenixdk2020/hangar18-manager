...
const H18_BASE_THEME_VERSION = '1.3.0';
...
function h18_base_theme_setup() {
    ...
    register_nav_menus(
        [
            'primary' => __('AKVPK primærmenu', 'akvpk'),
        ]
    );
}
...
function h18_base_get_update_manifest($force = false) {
    ...
    $response = wp_safe_remote_get(
        ...
        [
            'timeout'    => 15,
            'sslverify'  => true,
            'user-agent' => 'AKVPK-Theme/' . H18_BASE_THEME_VERSION . '; ' . home_url('/'),
            'headers'    => [
    ...
    $valid = (
        in_array($theme, ['hangar18-base', 'akvpk'], true) &&
...
function h18_base_theme_information($result, $action, $args) {
    ...
    return (object) [
        ...
        'author'        => '<a href="https://akvpk.dk/">AKVPK</a>',
...
function h18_base_pre_download_package($reply, $package, $upgrader, $hook_extra = []) {
    ...
    if ($manifest['package_encoding'] !== 'base64') {
        return new WP_Error(
            ...
            __('AKVPK-temapakken har et ukendt format.', 'hangar18-base')
        );
    }
    ...
    if ($encoded === false) {
        return new WP_Error(
            ...
            __('AKVPK-temapakken kunne ikke læses.', 'hangar18-base')
        );
    }
    ...
    if ($decoded === false) {
        return new WP_Error(
            ...
            __('AKVPK-temapakken kunne ikke afkodes.', 'hangar18-base')
        );
    }
    ...
    if (!hash_equals($manifest['package_sha256'], hash('sha256', $decoded))) {
        return new WP_Error(
            ...
            __('AKVPK-temapakkens kontrolsum passer ikke. Opdateringen er stoppet.', 'hangar18-base')
        );
    }

    $zip_file = wp_tempnam('akvpk-theme.zip');
    if (!$zip_file || file_put_contents($zip_file, $decoded) === false) {
        return new WP_Error(
            'hangar18_package_write_failed',
            __('AKVPK-temapakken kunne ikke gemmes midlertidigt.', 'hangar18-base')
        );
    }
...
function h18_base_admin_notice() {
    ...
    echo '<div class="notice notice-warning"><p><strong>AKVPK:</strong> ' .
        esc_html__('Visual Designer Manager er ikke aktiv. Temaet virker stadig, men den administrerede Hangar18-header/footer og specialfunktioner kan mangle.', 'hangar18-base') .
        '</p></div>';
}
