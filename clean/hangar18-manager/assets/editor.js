(function () {
    ...
    function normalizeProps(type, raw) {
        raw = raw && typeof raw === 'object' ? raw : {};
        if (type === 'text') {
            return { text: String(raw.text || 'Ny tekst'), align: ['left', 'center', 'right'].includes(raw.align) ? raw.align : 'left' };
        }
        if (type === 'image') {
    ...
    function reparent(id, parentId, dropGeometry) {
        const node = nodeById(id);
        if (!node) { return; }
        parentId = cleanId(parentId);
        const parent = parentId ? nodeById(parentId) : null;
    ...
    function renderSurface(parentId, surface) {
        ...
        if (!list.length) {
            ...
            empty.textContent = parentId ? 'Træk et eksisterende element ind i denne kasse' : 'Tilføj et element eller træk et element hertil';
        ...
        list.forEach(function (node) {
            ...
            move.draggable = true;
            move.title = 'Træk element ind i/ud af kasse';
            move.textContent = '✥';
            ...
            card.addEventListener('click', function (event) {
                if (event.target.closest('.h18-clean-resize,.h18-clean-move')) { return; }
                event.stopPropagation();
                selectedId = node.id;
                render();
            });
            card.querySelectorAll('.h18-clean-resize').forEach(function (handle) {
                handle.addEventListener('pointerdown', function (event) { beginResize(event, node.id, String(handle.getAttribute('data-resize') || ''), card, surface); });
            });
            ...
            copy.addEventListener('click', function () {
                ...
                if (navigator.clipboard && url) {
                    navigator.clipboard.writeText(url).then(function () {
                        const old = copy.textContent; copy.textContent = 'Link kopieret'; setTimeout(function () { copy.textContent = old; }, 1200);
                    });
                }
            });
        }
        render();
        diag('editor_boot', { version: CFG.version || '', state: structuralSummary() });
    }
